...
class PaintStream {
  ...
  reset() {
    this.x = Math.random() * canvas.width;
    this.y = -50; // Start higher above the canvas
  ...
  update() {
    ...
    for (let i = 0; i < this.particles.length; i++) {
      ...
      if (distance < mouseRadius) {
        ...
        particle.distortionX =
          Math.cos(angle) * force * 15 * distortionMultiplier;
        particle.distortionY =
          Math.sin(angle) * force * 8 * distortionMultiplier;
    ...
    if (this.y - this.particles[0].y > canvas.height) {
      this.reset();
    }
  }
  ...
  draw() {
    for (let i = 0; i < this.particles.length; i++) {
      ...
      const leftEdge =
        particleX - totalWidth / 2 + Math.sin(particleY * 0.1) * 2;
      const rightEdge =
        particleX + totalWidth / 2 + Math.sin(particleY * 0.1 + 1) * 2;
      ...
      ctx.quadraticCurveTo(
        ...
        cpY,
        rightEdge,
        particleY + particle.distortionY
      );
...
canvas.addEventListener("mousemove", (e) => {
  ...
  const distance = Math.sqrt(
    (mouseX - oldX) * (mouseX - oldX) + (mouseY - oldY) * (mouseY - oldY)
  );
