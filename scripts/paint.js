...
let config = {
  ...
  CURL: 50,  // Fixed at 50 for maximum vorticity
  SPLAT_RADIUS: 0.3,
  SPLAT_FORCE: 2000,
  SHADING: true,
  COLORFUL: true,
...
// Remove CURL from the GUI controls by modifying startGUI function
function startGUI() {
  var gui = new dat.GUI({ width: 300 });
  gui.hide();  // Hide the controls by default
  gui
...
function update() {
  ...
  if (!config.PAUSED) {
    step(dt);
    // Add streams every few frames
    if (Math.random() < 0.3) { // Adjust this value to control frequency
      createInitialSplats();
    }
  }
  render(null);
...
function createInitialSplats() {
  ...
  for (let i = 0; i < streams; i++) {
    const x = (i + 1) / (streams + 1); // Evenly space across width
    const color = config.STREAM_COLORS[i];
    splat(
      x,           // x position
      1,        // y position (very top)
      1,           // dx (no horizontal movement)
      -150,        // dy (gentler downward force)
      color
    );
  }
}
